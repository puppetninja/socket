...
#define UDPORT "3491"//The UDP port the user will use.
#define MESSAGENUM 2
...
int main(int argc, char *argv[])
{
    ...
    for(p = servinfo; p != NULL; p = p->ai_next) {
        if ((sockfd = socket(p->ai_family, p->ai_socktype,p->ai_protocol)) == -1) {
            perror("User#1: socket");
            continue;
        }

        if (connect(sockfd, p->ai_addr, p->ai_addrlen) == -1) {
            close(sockfd);
            perror("User#1: connect");
            continue;
        }
    ...
    printf("Phase 1:<User#1>Login request.User: %s  password: %s .\n", username, password);
   ...
   // loop through all the results and bind to the first we can
    ...
    if (p == NULL) {
         fprintf(stderr, "User#1: failed to bind socket\n");
         return 2;
    }
    freeaddrinfo(servinfoP3);//free the infomation of User1 on static UDP port.
    printf("Phase 1:<User#1> is listening on port number %s .\n",UDPORT);

    close(sockfd);//close TCP socket connection to the Login Server.

    printf("End of Phase 1 for <User#1>.\n");
    //To get the IP address of User#1 through UDP socket.
    if(getsockname(sockfdP3,(struct sockaddr *)&my_addr,&addrlen) == -1){
       	perror("User#1:getsockname");
       	exit(1);
       }
    printf("Phase 3: <User#1> has UDP port %s and IP address %s \n", UDPORT, inet_ntoa(my_addr.sin_addr));
    ...
    if (pFile == NULL){
         ...
         if (fgets (toUserMessage1 , MAXDATASIZE , pFileP3) == NULL ){
        	 ...
        	 if((numbytes = sendto(sockfdSP,toUserMessage1BK,strlen(toUserMessage1BK),0,p->ai_addr, p->ai_addrlen)) == -1){
        		 perror("User#1:send to Super Node");
        		 exit(1);
        	 }
        	 printf("Phase 3:<User#1> is sending the message: %s ",toUserMessage1BK);
         }
         ...
         if (fgets (toUserMessage2 , MAXDATASIZE , pFileP3) == NULL ){
        	 ...
        	 printf("Phase 3:<User#1> is sending the message: %s \n",toUserMessage2BK);
         }
    }
    ...
    //printf("User#1:waiting for the Super Node to send packet....");
    ...
    while(messageCounter != MESSAGENUM){
    	...
    	//pSeg = strtok(NULL," ");
    	//strcpy(content, pSeg);
    	printf("Phase 3:<User#1> received the message: %s \n",content);
    	messageCounter++;
    }
    ...
    if((numbytes = sendto(sockfdSP,termReq,strlen(termReq),0,p->ai_addr, p->ai_addrlen)) == -1){
        perror("User#1:send terminate request");
        exit(1);
    }
    //
    close(sockfdP3);
