...
#define UDPORT "3691"//The UDP port the user will use.
#define MESSAGENUM 2
...
int main(int argc, char *argv[])
{
    ...
    int messageCounter;
    char s[INET6_ADDRSTRLEN];//sP3[INET6_ADDRSTRLEN];
    char authen[5];
    char authenDeny[5];
    char userPass[80];
    char username[80];
    ...
    char fromUserMessage[MAXDATASIZE];//,fromUserMessage2[MAXDATASIZE];
    ...
    for(p = servinfo; p != NULL; p = p->ai_next) {
        if ((sockfd = socket(p->ai_family, p->ai_socktype,p->ai_protocol)) == -1) {
            perror("User#3: socket");
            continue;
        }

        if (connect(sockfd, p->ai_addr, p->ai_addrlen) == -1) {
            close(sockfd);
            perror("User#3: connect");
            continue;
        }
    ...
    printf("Phase 1:<User#3>Login request.User: %s  password: %s .\n", username, password);
   ...
   // loop through all the results and bind to the first we can
    ...
    if (p == NULL) {
         fprintf(stderr, "User#3: failed to bind socket\n");
         return 2;
    }
    freeaddrinfo(servinfoP3);//free the infomation of User1 on static UDP port.
    printf("Phase 1:<User#3> is listening on port number %s .\n",UDPORT);

    close(sockfd);//close TCP socket connection to the Login Server.

    printf("End of Phase 1 for <User#3>.\n");
    //To get the IP address of User#3 through UDP socket.
    if(getsockname(sockfdP3,(struct sockaddr *)&my_addr,&addrlen) == -1){
       	perror("User#3:getsockname");
       	exit(1);
       }
    printf("Phase 3: <User#3> has UDP port %s and IP address %s \n", UDPORT, inet_ntoa(my_addr.sin_addr));
    ...
    if (pFile == NULL){
         ...
         if (fgets (toUserMessage1 , MAXDATASIZE , pFileP3) == NULL ){
        	 ...
        	 if((numbytes = sendto(sockfdSP,toUserMessage1BK,strlen(toUserMessage1BK),0,p->ai_addr, p->ai_addrlen)) == -1){
        		 perror("User#3:send to Super Node");
        		 exit(1);
        	 }
        	 printf("Phase 3:<User#3> is sending the message: %s ",toUserMessage1BK);
         }
         ...
         if (fgets (toUserMessage2 , MAXDATASIZE , pFileP3) == NULL ){
        	 ...
        	 printf("Phase 3:<User#3> is sending the message: %s \n",toUserMessage2BK);
         }
    }
    freeaddrinfo(servinfoSP);
    //Wait for messages from SuperNode.
    //printf("before receiving stage of User1.\n");
    sleep(4);//to make a delay gap between the send and receive section.
    //printf("User#3:waiting for the Super Node to send packet....");
    ...
    while(messageCounter != MESSAGENUM){
    	...
    	//pSeg = strtok(NULL," ");
    	//strcpy(content, pSeg);
    	printf("Phase 3:<User#3> received the message: %s \n",content);
    	messageCounter++;
    }
    ...
    if((numbytes = sendto(sockfdSP,termReq,strlen(termReq),0,p->ai_addr, p->ai_addrlen)) == -1){
        perror("User#3:send terminate request");
        exit(1);
    }
    //
    close(sockfdP3);
    close(sockfdSP);
    printf("End of Phase 3 for <User#3>.\n");
    return 0;
}
